...
def validate_model_context_windows() -> None:
    ...
    client = OllamaClient(settings.LLM_ENDPOINT, settings.LLM_MODEL, settings.REQUEST_TIMEOUT_SECONDS)

    if settings.MODEL_CONTEXT_WINDOW <= 0:
        raise StartupValidationError("RH-MODEL-CONTEXT-MISMATCH", "MODEL_CONTEXT_WINDOW must be positive")

    actual_num_ctx = client.fetch_model_num_ctx(settings.LLM_MODEL)
    logger.info(
    ...
    for model_id in {settings.LLM_MODEL, settings.REWRITE_MODEL_ID}:
        provider_limit = client.fetch_model_num_ctx(model_id)
