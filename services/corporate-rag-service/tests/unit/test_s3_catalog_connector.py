...
def test_fcs_sp2_s3_descriptor_generation_pagination_sorted(monkeypatch):
    ...
    descriptors = connector.list_descriptors("t1", SyncContext(max_items_per_run=100, page_size=2, incremental_enabled=True))

    refs = [d.external_ref for d in descriptors]
    assert refs == ["s3:bucket:docs/a.md", "s3:bucket:docs/b.txt"]
    assert all(d.checksum_hint for d in descriptors)
...
def test_fcs_sp2_s3_fetch_item_positive(monkeypatch):
    ...
    connector = S3CatalogConnector(client=FakeS3Client())
    descriptor = connector.list_descriptors("t1", SyncContext(max_items_per_run=100, page_size=2, incremental_enabled=True))[0]
